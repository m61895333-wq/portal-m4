...
import { addTopicTag, createQueuedPost, deletePost, removeTopicTag, updatePost, setAutonomyStatus, listPosts, recordPageView } from "@/lib/portal-cms";
...
export async function remakePostAction(formData) {
  const id = String(formData.get("id"));
  let success = false;
  try {
    await updatePost(id, {
...
export async function remakeImageAction(formData) {
  const id = String(formData.get("id"));
...
export async function toggleAutonomyAction(formData) {
  try {
...
export async function recordHitAction(path) {
  try {
