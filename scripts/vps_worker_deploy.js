...
conn.on('ready', () => {
  console.log('VPS Connected via SFTP...');
  conn.sftp((err, sftp) => {
    if (err) throw err;
    
    const writeWorker = () => {
      const stream = sftp.createWriteStream('/opt/m4-worker/worker.py');
      stream.on('close', () => {
        console.log('worker.py uploaded.');
        writeService();
      });
      stream.write(pythonCode);
      stream.end();
    };

    const writeService = () => {
      const stream = sftp.createWriteStream('/etc/systemd/system/m4-worker.service');
      stream.on('close', () => {
        console.log('m4-worker.service uploaded.');
        enableService();
      });
      stream.write(serviceCode);
      stream.end();
    };

    const enableService = () => {
      conn.exec('systemctl daemon-reload && systemctl enable m4-worker && systemctl start m4-worker', (err2, stream2) => {
        if (err2) throw err2;
        stream2.on('close', () => {
