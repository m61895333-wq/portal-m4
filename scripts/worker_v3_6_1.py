...
def get_autonomy_config():
    try:
        res = supabase.table('portal_settings').select('*').eq('key', 'autonomy_status').execute()
        if res.data:
...
def process_queue():
    ...
    for post in posts:
        post_id = post['id']
        topic = post['title']
        print(f'>>> PRODUCAO INICIADA (v3.6.1): {topic}')
        ...
        if content:
            ...
            supabase.table('portal_posts').update({
                ...
                'reviewer_notes': f"SEO M4 Ativo | Autonomia v3.6.1 | Keywords Visual: {visual_keywords}"
...
if __name__ == "__main__":
    print("M4 SOVEREIGN WORKER v3.6.1 - ESTABILIZADO")
    while True:
        try:
            process_queue()
        except Exception as e:
